...
def get_patients(patients_num):
    """

    :param patients_num: [1, .., 30]
    :return: patients
    """
    patient_params = pd.read_csv(PATIENT_PARA_FILE)

    patients = [(patient_params['Name'][p - 1]) for p in patients_num]
    return patients
...
def get_insulin_pump(selection):
